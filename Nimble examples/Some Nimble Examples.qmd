...
X4 = rbinom(N, size = 3, prob = 0.5) + 1

coef_X4 = case_when(X4 == 1 ~ 0, 
                    X4 == 2 ~ 0.1, 
                    X4 == 3 ~ 1, 
                    X4 == 4 ~ 1.3)

Y = 1 - 1 * X1 + 2 * X2 + 0.5 * X3 +  coef_X4
...
snimble_example2 <- 
  nimbleCode({
    ...
    for(i in 1:N){
      pi[i] <- expit(beta0 + beta1 * x1[i] + beta2 * x2[i] + beta3 * x3[i] )
...
mcmc.out <- nimbleMCMC(code = snimble_example2, 
                       ...
                       data = data_logistic, 
                       inits = Inits, 
                       monitors = c("beta0", "beta1", "beta2", "beta3", "outcome", "pi"), 
                       thin = 1,
                       niter = 10000, 
                       nburnin = 10000/10, 
                       nchains = 1, 
...
freq_glm <- glm(data_logistic$outcome ~ data_logistic$x1 + data_logistic$x2 + data_logistic$x3, family = "binomial")

freq_glm %>% tidy() %>% 
  select(-statistic, - p.value, -term) %>%
  mutate(term = c("(Intercept)", "x1", "x2", "x3")) %>% 
