...
q_norm_b  = 1
# q_norm_zi = 0.005
q_norm_zi = rep(1, length(zi_guess)) # abs(zi_guess)*5 # make it such that the step size for each z_ia is poportional to its data-driven estiamte
#                             ratio is 1/1
...
final_lambdas <- apply(lambdas, MARGIN = 1, FUN = median)
final_lambdas_q025 <- apply(lambdas, MARGIN = 1, FUN = function(x){quantile(x, 0.025)})
final_lambdas_q975 <- apply(lambdas, MARGIN = 1, FUN = function(x){quantile(x, 0.975)})
...
if(T == F){
  ...
  ggplot(data = data.frame(my = aa.med, 
                           hq = aa.med2), 
         aes(x = my, y = hq)) + 
    geom_point() + 
    ...
    ggtitle(paste0(
      "Coef: ", round(
        lm(aa.med2~aa.med) %>% broom::tidy() %>% {.[2,2]} %>% unlist(), 2
      )
    )) 
