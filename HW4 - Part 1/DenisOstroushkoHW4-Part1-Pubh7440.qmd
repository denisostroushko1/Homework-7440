...
\begin{align*}
  ...
  \Pi_{i,a} \left[ Pois(Y_{ia} | n_{ia} * exp(\beta_{0a} + z_{ia})) \right]   \hspace{1cm} \textcolor{blue}{full \ data \ likelihood} \\ 
  & Norm(\beta_{0a} | 0, \tau^2_a) \hspace{2cm} \textcolor{blue}{prior \ for \ \beta_{0a}} \\
  & Norm(z_{ia} | 0, \sigma^2_a) \hspace{2cm} \textcolor{blue}{prior \ for \ z_{ia}} \\ 
  & IG(\sigma^2_a | 0.001, 0.001) \hspace{1.5cm} \textcolor{blue}{prior \ for \ \sigma^2_a}
\end{align*}
...
* Assume an asymmetric candidate density $\sigma^2_a$ ~ $IG(q, q * \sigma^{`2}_a)$, where q = 3 and $\sigma^{`2}_a$ is the 
  most recent updated value of $\sigma^2_a$ from the Metropolis-Hastings iteration 
...
sigma_0a <- 
  ...
  )## iniitiate sigma 2 for three age groups at 1,1,1. Not sure if these values even matter 
