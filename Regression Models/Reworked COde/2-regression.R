...
prior_intercept <- normal(1000, 1)  # Normal distribution with mean 0 and standard deviation 10
prior_slope <- student_t(2000, 3, .5)  # Student's t distribution with 3 degrees of freedom, mean 0, and scale 2.5

# Fit the model with specified priors
model_bayes <- stan_glm(Y ~ x, seed=111, prior = c(prior_intercept, prior_slope))


print(model_bayes, digits = 3)

mcmc_dens(model_bayes, pars = c("x"))+
  vline_at(model_bayes$coefficients['x'], col="red")
