...
$\theta_{ia} | \beta_{0a}, \sigma^2_a$ ~ $N(\beta_{0a}, \sigma^2_a)$. This equation represents 201 distributions for each county. Within an age group, log death rates for across counties are iid normal random variables. 

$\beta_{0a} | \mu = 0, \tau^2_a$ ~ $N(0, \tau^2_a)$, where $\tau^2 = 10,000$. So, the prior distribution of the average of log death rates of a given age group is non-informative. 
...
* Will use 50,000 iterations for the loop to obtain posterior samples, and will use iterations 1 to 10,000 as burnin. These iterations will be discarded and will not contribute to the final distribution. 
  + I suspect that working with rejection rates and optimizing $q$ and $q_{ia}$ for my algorithm will result in a more efficient 
    sampling procedure and will reduce the number of replications required. 
...
@fig-map  presents county-specific age-adjusted mortality rates per 100,000 using regression with age as a predictor. Overall, the map looks similar to what we observed under the Poisson-Gamma model (HW3). @fig-map-p1 shows results obtained in part 1, 
...
@fig-beta0-sum-comp presents posterior distribution for the average log-death rate for each group. Top row represents estimates obtained from part 1 , and bottom row are part 2 estimates (this HW). 
