---
title: "Denis Ostroushko - PUBH 7440 - HW4 - Part 2s"
format: pdf
execute: 
  warning: false
  message: false 
...
q_norm_b  = 0.075
# q_norm_zi = 0.005
q_norm_theta_ai =1 # make it such that the step size for each z_ia is poportional to its data-driven estiamte
...
for(i in 2:reps){
  ...
  for(POP in 1:3){
    ...
    ratio  <- 
      # -1/(2 * sigma_calc) * (sigma_calc - sum_sq_w_curr) - 
      -1/(2 * sigma_calc) * (sigma_calc - sum_sq_w_curr) - 
  ...
  ratio <- 
    ...
    1/(2 * sig2_calc) * (theta_ia_prop^2 - theta_ia_curr^2) + 
    b_0a_calc / (2 * sig2_calc) * (theta_ia_prop - theta_ia_curr) 
